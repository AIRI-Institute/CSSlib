...
class Calculator:
    ...
    def __init__(self, data: str | DataLoader, inputs: InputSet | Any, calculations_path: str, cmd: str | list[str],
                 ...
                 use_sftp: bool = False, use_local: bool = False, **dataloader_fields):
        """
            ...
            Args:
                ...
                use '#' symbol before the cmd. If use_local flag is True and cmd is a list[str] then the last command will be associated with local machine.
                ...
                use_sftp (bool, optional): if True sftp tunnel will be used for files sharing with remote machines else scp will be used. Defaults to False.
    ...
    def __validate_init_parameters(self):
        ...
        if isinstance(self.__server_ip, str):
            ...
            if isinstance(self.__cmd, list) and (len(self.__cmd) != len(self.__server_ip) or (self.__use_local and len(self.__cmd) != len(self.__server_ip) + 1)):
    ...
    def __prepare_init_parameters(self):
        ...
        if isinstance(self.__server_ip, list):
            ...
            if isinstance(self.__cmd, list) and not self.__use_local:
                self.__cmd = {server_ip: self.__cmd[indx] for indx, server_ip in enumerate(self.__server_ip)}
            elif isinstance(self.__cmd, list) and self.__use_local:
                local_cmd = self.__cmd[-1]
                self.__cmd = {server_ip: self.__cmd[indx] for indx, server_ip in enumerate(self.__server_ip)}
                self.__cmd['local'] = local_cmd
    ...
    def run(self):
        ...
        for data in self.__data.itertuples(index=True):
            # data.Index
            # data.cif_data
            ...
